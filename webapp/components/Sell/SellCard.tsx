...
const SellCard = ({ nft, i, setSelectedNft }: any) => {
  return (
    ...
    >
      <ThirdwebNftMedia className="rounded-md group-hover:drop-shadow-[0px_0px_10px_#d1d1d1] ease-in duration-300" metadata={nft?.metadata} />
      {nft.type === "ERC1155" && (
        <p className="absolute top-3 left-3 text-lg tracking-widest px-2 py-1 bg-moon-gold bg-opacity-80 rounded-xl">{"x" + nft.quantityOwned}</p>
      )}
